...
- Cada execução deste prompt processa UM ÚNICO documento de cada vez (um PDF = um contrato, um aditivo ou uma Ordem de Serviço).
- Todo documento fornecido será, exclusivamente, um CONTRATO, um ADITIVO DE CONTRATO ou uma ORDEM DE SERVIÇO (OS).
- Um ADITIVO deve ser tratado como uma linha totalmente separada e independente, com seus próprios campos extraídos exclusivamente a partir do conteúdo do próprio aditivo.

IDENTIFICAÇÃO DO TIPO DE DOCUMENTO
- Identifique se o documento é um CONTRATO ou um ADITIVO e preencha o campo "tipo_documento" com "CONTRATO" ou "ADITIVO". Uma Ordem de Serviço (OS) não é, ela própria, um "tipo_documento" — ela só é usada como fonte de data para os campos abaixo quando indicado.
- Se for um ADITIVO, o campo "numero_contrato" deve conter o número do contrato principal seguido da identificação do aditivo, no formato "NÚMERO DO CONTRATO – ADITIVO Nº XX" (ex.: "ENG 778/2023 – ADITIVO Nº 01"). Se o aditivo não tiver número, use "ADITIVO" sem numeração.
- Se for um CONTRATO, o campo "numero_contrato" deve conter apenas o número do contrato.

REGRAS GERAIS
1. Analise integralmente o documento enviado para esta execução.
2. Considere, além do documento principal, os anexos, DocuSign e demais páginas que compõem ESTE mesmo arquivo PDF. Caso o mesmo arquivo PDF contenha, em páginas distintas, tanto o CONTRATO/ADITIVO quanto a ORDEM DE SERVIÇO (OS) — cada trecho identificado pelo marcador "=== {{NOME DO DOCUMENTO}} — PÁGINA {{N}} ===" —, trate cada trecho conforme as regras específicas de cada campo abaixo.
3. Para cada informação extraída, inclua no próprio texto do campo a origem do dado: cláusula (número e título, quando houver), anexo (quando aplicável) e a página do documento onde a informação foi localizada. As páginas estão marcadas no texto recebido com "=== {{NOME DO DOCUMENTO}} — PÁGINA {{N}} ==="; use esse nome de documento e número de página nas citações.
4. Quando uma informação estiver distribuída em mais de uma cláusula ou página, cite todas as referências utilizadas.
5. Não faça suposições nem preencha informações por dedução. Extraia apenas dados expressamente previstos no documento, ou realize cálculos apenas quando houver regra específica para isso.
6. Caso uma informação não exista ou não seja localizada, preencha o campo com "{NAO_LOCALIZADO}".
...
- Tipo do documento: informe "CONTRATO" ou "ADITIVO", conforme identificado. Se o documento em análise for exclusivamente uma Ordem de Serviço (OS), sem conteúdo de contrato/aditivo, informe "{NAO_LOCALIZADO}".
- Número do contrato: se CONTRATO, informe o número do contrato. Se ADITIVO, informe "NÚMERO DO CONTRATO – ADITIVO Nº XX" (ou "– ADITIVO" se não houver numeração), citando cláusula/página onde a identificação foi localizada.
- Data da assinatura do documento: se houver informações de DocuSign, use a Data de Conclusão (Completed Date) do DOCUMENTO em análise (contrato ou aditivo). Caso não exista DocuSign, localize a página com as assinaturas do CONTRATO/ADITIVO e use a data ali constante. Analisar apenas o trecho que contém o nome "CONTRATO" ou "ADITIVO" para esse campo. NUNCA usar o trecho/documento que contém o nome "ORDEM DE SERVIÇO" ou "OS" para esse campo — se o documento em análise for exclusivamente uma OS, preencha este campo com "{NAO_LOCALIZADO}".
- Data de início da vigência: se o CONTRATO estabelecer que a vigência (ou o prazo de execução) se inicia na data de assinatura da Ordem de Serviço (OS), localize a data de assinatura da OS (Completed Date do DocuSign da OS, ou a data da página de assinaturas da OS caso não haja DocuSign) e utilize-a — analisando, quando ambos estiverem presentes neste mesmo arquivo, o trecho do CONTRATO (para confirmar a regra) em conjunto com o trecho da OS (para obter a data). Caso o documento informe uma data de início específica (independente de OS), utilize a data expressamente indicada. Se o CONTRATO fizer referência a uma OS que não está presente neste mesmo arquivo, informe "{NAO_LOCALIZADO}" e cite a cláusula/página onde essa condição está prevista. NUNCA analisar o trecho/documento que contém o nome "ADITIVO" para esse campo.
- Data final da vigência: identifique o prazo de vigência previsto (ex.: 12 meses, 365 dias, 24 meses). Se a vigência terminar após esse prazo contado da data de assinatura da OS, e essa data estiver disponível neste mesmo arquivo, calcule a data final somando o prazo à data de assinatura da OS. Caso o documento informe uma data final específica, utilize-a diretamente. No documento que contém o nome "ADITIVO", se tratar apenas de prorrogação do prazo, trazer apenas a data final informada no aditivo.
- Prazo de vigência: informe o prazo previsto (dias, meses ou anos), citando cláusula, anexo e página. Em aditivos que alteram prazos, informe o novo prazo estabelecido pelo aditivo. Em aditivos de prorrogação, calcule a nova data final conforme o novo prazo estabelecido pelo aditivo.
- Período de medição mensal do contrato: informe a data inicial/final do período, periodicidade, data limite para apresentação da medição e cláusula/página. Caso não haja previsão, informe "Período de medição não previsto." Analisar apenas o trecho que contém o nome "CONTRATO" ou "ADITIVO" para esse campo. NUNCA usar o trecho/documento que contém o nome "ORDEM DE SERVIÇO" ou "OS" para esse campo — se o documento em análise for exclusivamente uma OS, preencha este campo com "Período de medição não previsto."
- Arquivo(s) de origem: informe o nome do arquivo analisado.
